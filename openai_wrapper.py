...
class CustomChatCompletions:
    ...
    def create(self, *args, **kwargs) -> ChatCompletion | Stream[ChatCompletionChunk]:
        ...
        if not stream and tools:
            ...
            adapted_response = CustomLLMResponseAdapter.adapt_response(cast(str, response.choices[
                0].message.content))  # return a new ChatCompletion with function callings inside
...
class CustomOpenAIClient(OpenAI):
    def __init__(self, *args, **kwargs):
        ...
        self.chat.completions = cast(Completions,
                                     CustomChatCompletions(self.chat.completions, debug=False))  # type: ignore
...
class AsyncCustomChatCompletions:
    ...
    async def create(self, *args, **kwargs) -> ChatCompletion | AsyncStream[ChatCompletionChunk]:
        ...
        if not stream and tools:
            ...
            if not response.choices[0].message.content:  # GPT function does not have this value
                return response
...
class AsyncCustomOpenAIClient(AsyncOpenAI):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.chat.completions = cast(AsyncCompletions,
                                     AsyncCustomChatCompletions(self.chat.completions, debug=False))  # type: ignore
